...
7. **Clean installation:** the **"Remove llama.cpp"** button deletes `~/llama.cpp` and the installed `~/.local/bin/llama-server` after confirmation, so switching versions leaves no old source/build artifacts. It never removes `~/.llamatray` settings or profiles and asks you to stop a running server first.
...
7. **Temiz kurulum:** **"llama.cpp'ı Temizle"** butonu onaydan sonra `~/llama.cpp` klasörünü ve kurulu `~/.local/bin/llama-server` dosyasını siler; sürüm değiştirirken eski kaynak/build kalıntısı bırakmaz. `~/.llamatray` ayar ve profillerine kesinlikle dokunmaz, çalışan sunucuyu önce durdurmanızı ister.
