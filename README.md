...
4. **Option A — kaynaktan derleme:** llama.cpp'ı `~/llama.cpp`'a klonlar, **temiz derleme** yapar (derlemeyi başlattığınızda veya backend'i değiştirdiğinizde `~/llama.cpp/build` otomatik silinir), CMake'ı seçilen backend açık ve diğerleri açıkça kapalı şekilde yapılandırır (ör. CPU: `-DGGML_VULKAN=OFF -DGGML_CUDA=OFF -DGGML_HIPBLAS=OFF -DGGML_SYCL=OFF`), `llama-server`'ı tüm çekirdeklerle derler ve sonucu `~/.local/bin`'e symlink olarak kurar. Seçili backend'in SDK'sı (nvcc/hipcc/icpx) eksikse derlemeden önce uyarı gösterilir. Klonlama sonrası en güncel GitHub release `tag_name` değeri alınır ve sürüm etiketindeki geçici kısa commit tanımlayıcısının yerini alır; ağ yoksa yerel kaynak tanımlayıcısı fallback olarak kalır.
