...
3. **Dependencies:** the manager detects your package manager (apt/dnf/pacman/zypper) and shows a *dynamic* dependency list for the selected backend (e.g. Vulkan adds `libvulkan-dev`, `vulkan-tools`, `glslc` on apt; CUDA requires `nvcc`; ROCm requires `hipcc`; SYCL requires `icpx`). Missing base/backend packages are installed with one click using explicit package lists, not group names. If a large GPU SDK (CUDA/ROCm/SYCL) is missing, the manager explains that it must be installed manually — or suggests Option B.
4. **Option A — compile from source:** clones llama.cpp into `~/llama.cpp`, performs a clean build (removes `~/llama.cpp/build` whenever you start a build or switch backends), configures CMake with the selected backend enabled and all others explicitly disabled (e.g. CPU: `-DGGML_VULKAN=OFF -DGGML_CUDA=OFF -DGGML_HIPBLAS=OFF -DGGML_SYCL=OFF`), builds `llama-server` using all cores, and symlinks the result into `~/.local/bin`. A pre-flight warning is shown before building if the selected backend's SDK is missing.
...
3. **Bağımlılıklar:** yönetici paket yöneticinizi (apt/dnf/pacman/zypper) tespit eder ve seçilen backend'e göre **dinamik** bir bağımlılık listesi gösterir (ör. Vulkan apt'te `libvulkan-dev`, `vulkan-tools`, `glslc` ekler; CUDA için `nvcc`, ROCm için `hipcc`, SYCL için `icpx` gereklidir). Eksik temel/backend paketleri grup adı değil açık paket listesiyle tek tıkla kurulabilir. Büyük bir GPU SDK'sı (CUDA/ROCm/SYCL) eksikse yönetici manuel kurulum gerektiğini bildirir — ya da Option B'yi önerir.
4. **Option A — kaynaktan derleme:** llama.cpp'ı `~/llama.cpp`'a klonlar, **temiz derleme** yapar (derlemeyi başlattığınızda veya backend'i değiştirdiğinizde `~/llama.cpp/build` otomatik silinir), CMake'ı seçilen backend açık ve diğerleri açıkça kapalı şekilde yapılandırır (ör. CPU: `-DGGML_VULKAN=OFF -DGGML_CUDA=OFF -DGGML_HIPBLAS=OFF -DGGML_SYCL=OFF`), `llama-server`'ı tüm çekirdeklerle derler ve sonucu `~/.local/bin`'e symlink olarak kurar. Seçili backend'in SDK'sı eksikse derlemeden önce uyarı gösterilir.
