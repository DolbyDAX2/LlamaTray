...
3. **Dependencies:** the manager detects your package manager (apt/dnf/pacman/zypper) and shows a *dynamic* dependency list for the selected backend. **"Install Missing" runs the full development set for that backend** with explicit package lists — e.g. Vulkan on apt: `build-essential cmake git libvulkan-dev vulkan-tools glslc spirv-headers`; NVIDIA CUDA on apt: `+ nvidia-cuda-toolkit nvidia-cuda-dev libcuda1` (dnf: `xorg-x11-drv-nvidia-cuda-devel cuda-toolkit`, pacman: `cuda nvidia-utils`); AMD ROCm: `rocm-hip-sdk hipcc rocminfo` / `rocm-hip-devel rocminfo`. Installation runs through **`pkexec`** so the system's *graphical* password dialog appears (falling back to `sudo` when pkexec is unavailable).
...
3. **Bağımlılıklar:** yönetici paket yöneticinizi (apt/dnf/pacman/zypper) tespit eder ve seçilen backend'e göre **dinamik** bir bağımlılık listesi gösterir. **"Eksikleri Yükle" butonu, o backend'in tüm geliştirme paketlerini** açık paket listesiyle otomatik kurar — ör. Vulkan (apt): `build-essential cmake git libvulkan-dev vulkan-tools glslc spirv-headers`; NVIDIA CUDA (apt): `+ nvidia-cuda-toolkit nvidia-cuda-dev libcuda1` (dnf: `xorg-x11-drv-nvidia-cuda-devel cuda-toolkit`, pacman: `cuda nvidia-utils`); AMD ROCm: `rocm-hip-sdk hipcc rocminfo` / `rocm-hip-devel rocminfo`. Kurulum **`pkexec`** üzerinden çalıştırılır; böylece sistemin *grafiksel* şifre istemi diyalog kutusu açılır (pkexec yoksa `sudo`'ya düşer).
